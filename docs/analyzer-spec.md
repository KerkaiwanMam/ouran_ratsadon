...
- **Source of truth**: `apps/web/data/budget-XXXX.json` files, one per fiscal year, committed to the repo (and regenerated by the ETL step on each admin upload).
- **In-memory cache** (`lib/civic-cache.ts`): lazily loaded per fiscal year on first request directly from `apps/web/data/budget-XXXX.json` (`JSON.parse`), then cached in a `Map`. Serves **all** civic reads — `/explore` treemap/sunburst, drill-down breadcrumb, year-over-year compare, red-flag aggregation, AND `/api/civic/search` / `/api/civic/export/*` (filters, sort, pagination all run in-memory over the cached array). The dataset is small (≤ a few MB per year, ~4 years = single-digit MB total), so this is correct at this scale.
- **Postgres `BudgetLineItem`**: flat, 1-row-per-line-item table populated by bulk insert during admin upload (see database-schema.md). The ETL step reads these rows back (`findMany({ where: { fiscalYear } })`) to aggregate into the `CivicBudgetYear` JSON tree written to `apps/web/data/budget-XXXX.json`. On delete/replace, rows are removed via `deleteMany({ where: { fiscalYear } })`. This table is the durable raw-data audit trail and ETL source — not a serving store.
...
2. Run Python script to parse → produces `apps/web/data/budget-XXXX.json`
3. Team reviews JSON for parsing errors before publish
4. Admin uploads via `/admin/civic-data` → raw rows are bulk-inserted into `BudgetLineItem`, then the ETL step aggregates them into a `CivicBudgetYear` tree, writes `apps/web/data/budget-XXXX.json`, and invalidates the in-memory cache for that fiscal year
